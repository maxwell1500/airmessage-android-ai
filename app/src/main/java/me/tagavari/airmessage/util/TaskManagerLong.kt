...
class TaskManagerLong<T : Any> {
	private val requestMap = LongSparseArray<SingleSubject<T>>()
	 ...
	 */
	@CheckReturnValue
	fun run(id: Long, taskSupplier: Supplier<Single<T>>): SingleSubject<T> {
	 ...
	 */
	@CheckReturnValue
	operator fun get(id: Long): SingleSubject<T>? {
