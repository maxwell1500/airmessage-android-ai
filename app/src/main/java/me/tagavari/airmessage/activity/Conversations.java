...
public class Conversations extends AppCompatCompositeActivity {
    ...
    private final Runnable timeUpdateHandlerRunnable = new Runnable() {
        @Override
        public void run() {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        //Apply proper theming to app bar for all dark themes (not just AMOLED)
        applyProperTheming();
        ...
        if (event instanceof ReduxEventMessaging.Message) {
            ...
            getConversationFromAction(memberEvent, (conversation, i) -> {
                if (memberEvent.isJoin())
                    conversation.getMembers().add(memberEvent.getMember().clone());
                else
                    conversation.getMembers().removeIf(member -> member.getAddress().equals(memberEvent.getMember().getAddress()));
            ...
            for (ListIterator<ConversationInfo> iterator = viewModel.conversationList.listIterator(); iterator.hasNext(); ) {
                ...
                if (conversationInfo.getServiceHandler() != deleteEvent.getServiceHandler())
                    continue;
            ...
            getConversationFromAction(draftFileEvent, (conversation, i) -> {
                //Updating the conversation
                if (draftFileEvent.isAddition())
                    conversation.getDraftFiles().add(draftFileEvent.getDraft());
                else
                    conversation.getDraftFiles().removeIf(draft -> draft.getLocalID() == draftFileEvent.getDraft().getLocalID());
    ...
    void setSearchState(boolean enabled, boolean updateContext) {
        ...
        if (enabled) {
            //Hiding the toolbar
            if (updateContext)
                viewToolbar.animate().alpha(0).setDuration(duration).withEndAction(() -> viewToolbar.setVisibility(View.GONE));
            ...
            //Showing the toolbar
            if (updateContext)
                viewToolbar.animate().alpha(1).setDuration(duration).withStartAction(() -> viewToolbar.setVisibility(View.VISIBLE));
    ...
    private void updateMarkAllRead() {
        if (menuItemMarkAllRead == null || viewModel.stateLD.getValue() != ActivityViewModel.stateReady)
            return;
    ...
    private void promptSync() {
        ...
        if (!pluginCS.getConnectionManager().isPendingSync() || viewModel.stateLD.getValue() != ActivityViewModel.stateReady || getSupportFragmentManager().findFragmentByTag(keyFragmentSync) != null)
            return;
    ...
    void applyProperTheming() {
        // Apply proper app bar theming for all dark themes, not just AMOLED
        if (ThemeHelper.isNightMode(getResources()) && !ThemeHelper.shouldUseAMOLED(this)) {
            // Get the theme's background color using attribute resolution
            TypedValue typedValue = new TypedValue();
            
            // Try to resolve colorSurface first (for custom themes)
            if (getTheme().resolveAttribute(com.google.android.material.R.attr.colorSurface, typedValue, true)) {
                int backgroundColor = typedValue.data;
                findViewById(R.id.appbar).setBackgroundColor(backgroundColor);
                findViewById(R.id.viewgroup_search).setBackgroundColor(backgroundColor);
            } else {
                // Fallback to colorToolbar for standard dark theme
                int darkBackgroundColor = getColor(R.color.colorToolbar);
                findViewById(R.id.appbar).setBackgroundColor(darkBackgroundColor);
                findViewById(R.id.viewgroup_search).setBackgroundColor(darkBackgroundColor);
            }
        }
    }
    ...
    private class ConversationRecyclerAdapter extends RecyclerView.Adapter<VHConversationDetailed> {
        ...
        public void onBindViewHolder(@NonNull VHConversationDetailed holder, int position) {
            ...
            //Apply custom theme colors to conversation list item
            VBConversation.applyCustomThemeColors(Conversations.this, holder.itemView, holder.getConversationTitle(), holder.getLabelMessage(), holder.getLabelStatus());
    ...
    private class SearchRecyclerAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
        ...
        public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
            switch (getItemViewType(position)) {
                case itemTypeSubheader:
                    ...
                    if (position == 0 && !conversationFilterList.isEmpty())
                        ((SubheaderViewHolder) holder).label.setText(R.string.part_conversations);
        ...
        public int getItemViewType(int position) {
            if (position == 0)
                return itemTypeSubheader; //The first position will always be a header
            if (conversationFilterList.isEmpty())
                return itemTypeMessage; //If there are no conversations listed and the position is not 0, the item must be a message
            if (position - 1 < conversationFilterList.size())
                return itemTypeConversation; //If the position is not 0 and is less than the size of the conversation list, it must be a conversation
            if (position - 1 == conversationFilterList.size())
                return itemTypeSubheader; //If the position is the same size as the conversation list, it reaches the first entry of of the "messages" section, and is therefore the messages section subheader
