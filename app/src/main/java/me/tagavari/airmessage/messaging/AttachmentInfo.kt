...
class AttachmentInfo : MessageComponent {
	...
	fun <T : FileDisplayMetadata> getDisplayMetadata(supplier: Supplier<Single<T>>): Single<T> {
		val subject = displayMetadataSubject
		if(subject != null) return subject as Single<T>
		val newSubject = SingleSubject.create<T>()
		displayMetadataSubject = newSubject as SingleSubject<FileDisplayMetadata>
		supplier.get().subscribe(newSubject)
		return newSubject as Single<T>
	}
