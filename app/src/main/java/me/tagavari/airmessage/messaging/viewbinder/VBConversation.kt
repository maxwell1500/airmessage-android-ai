...
object VBConversation {
	...
	/**
	 * Applies custom theme styling to conversation list items
	 * @param context The context to use
	 * @param itemView The main conversation item view
	 * @param labelTitle The conversation title TextView
	 * @param labelMessage The conversation message TextView 
	 * @param labelStatus The conversation status TextView
	 */
	@JvmStatic
	fun applyCustomThemeColors(context: Context, itemView: View, labelTitle: TextView, labelMessage: TextView, labelStatus: TextView) {
		val currentTheme = me.tagavari.airmessage.helper.ThemeHelper.getCurrentTheme(context)
		if (me.tagavari.airmessage.helper.ThemeHelper.isCustomTheme(currentTheme)) {
			// Apply custom theme background gradient to the conversation item
			when (currentTheme) {
				me.tagavari.airmessage.helper.ThemeHelper.themeLuxurious -> {
					if (me.tagavari.airmessage.helper.ThemeHelper.isNightMode(context.resources)) {
						itemView.setBackgroundResource(me.tagavari.airmessage.R.drawable.gradient_luxurious_dark)
					} else {
						itemView.setBackgroundResource(me.tagavari.airmessage.R.drawable.gradient_luxurious_light)
					}
					// Apply luxurious theme text colors - use primary colors for better visibility against gradient
					labelTitle.setTextColor(context.resources.getColor(me.tagavari.airmessage.R.color.luxurious_primary, null))
					labelMessage.setTextColor(resolveColorAttr(context, android.R.attr.textColorPrimary))
					labelStatus.setTextColor(resolveColorAttr(context, android.R.attr.textColorSecondary))
				}
				me.tagavari.airmessage.helper.ThemeHelper.themeOceanBreeze -> {
					if (me.tagavari.airmessage.helper.ThemeHelper.isNightMode(context.resources)) {
						itemView.setBackgroundResource(me.tagavari.airmessage.R.drawable.gradient_ocean_dark)
					} else {
						itemView.setBackgroundResource(me.tagavari.airmessage.R.drawable.gradient_ocean_light)
					}
					// Apply ocean breeze theme text colors - use primary colors for better visibility against gradient
					labelTitle.setTextColor(context.resources.getColor(me.tagavari.airmessage.R.color.ocean_primary, null))
					labelMessage.setTextColor(resolveColorAttr(context, android.R.attr.textColorPrimary))
					labelStatus.setTextColor(resolveColorAttr(context, android.R.attr.textColorSecondary))
				}
				me.tagavari.airmessage.helper.ThemeHelper.themeSunsetGlow -> {
					if (me.tagavari.airmessage.helper.ThemeHelper.isNightMode(context.resources)) {
						itemView.setBackgroundResource(me.tagavari.airmessage.R.drawable.gradient_sunset_dark)
					} else {
						itemView.setBackgroundResource(me.tagavari.airmessage.R.drawable.gradient_sunset_light)
					}
					// Apply sunset glow theme text colors - use primary colors for better visibility against gradient
					labelTitle.setTextColor(context.resources.getColor(me.tagavari.airmessage.R.color.sunset_primary, null))
					labelMessage.setTextColor(resolveColorAttr(context, android.R.attr.textColorPrimary))
					labelStatus.setTextColor(resolveColorAttr(context, android.R.attr.textColorSecondary))
				}
			}
		} else {
			// Reset to default styling for non-custom themes
